...
filenames = ['planet_of_the_damned.txt', 'crisis_on_titan.txt', 'the_dancers.txt']


def word_count(filename, word):
    with open(filename) as f:
        content = f.read()
        word_counted = content.lower().count(word)
        print(f"the word \'{word}\' appears {word_counted} times in {filename}")
